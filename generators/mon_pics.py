...
class MonPicsGenerator(BaseGenerator):
    def generate(self, env):
        ...
        self.generate_mon_pics(self.core_data["mon_front_pics"], self.core_data["species_to_national"], "front_shiny", (0, 0, 64, 64), self.core_data["mon_shiny_palettes"])
        self.generate_mon_pics(self.core_data["mon_back_pics"], self.core_data["species_to_national"], "back_shiny", (0, 0, 64, 64), self.core_data["mon_shiny_palettes"])
    ...
    def get_correct_filepath(self, filepath, filetype, new_filetype):
        """
        Process the mon's gfx filepath while taking into account the special paths for Castform and Cherrim.
        """
        result = re.sub(filetype, new_filetype, filepath)
        if not os.path.exists(result):
            result = re.sub(r"(.*)/(.*)", r"\1/normal/\2", filepath)
            result = re.sub(filetype, new_filetype, result)
        return result
    ...
    def generate_mon_pics(self, species_to_pics, species_to_national, name, crop, mon_palettes=[], force=False):
        ...
        for species in species_to_pics:
            ...
            png_filepath = self.get_correct_filepath(species_to_pics[species], r"\.4bpp.*", ".png")
            ...
            if force or not os.path.exists(dest_filepath):
                if not os.path.exists(png_filepath):
                    ...
                    if mon_palettes is not [] and img.mode == "P" and species in mon_palettes and os.path.exists(mon_palettes[species]) and (force or not os.path.exists(dest_filepath)):
                        palette_filepath = self.get_correct_filepath(mon_palettes[species], r"\.gbapal.*", ".pal")
